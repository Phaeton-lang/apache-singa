...
def extract_and_plot(log_file_name, net_name, time_scale=1000.0, mem_scale=1024.0):
    ...
    with open(log_file_name, 'r') as mem_info_reader:
        ...
        for info in info_list:
            ...
            if counter == 0:
                ...
                if it_list[0] == 'MALLOC:':
                    # in KB
                    mem_addr = (int(it_list[1], 16) - start_mem_addr)/time_scale
                    malloc_size = float(it_list[2])/1024.0
                    max_mem_addr = max(max_mem_addr, mem_addr + malloc_size)
                    # in ms
                    malloc_time_stamp = (int(it_list[3]) - start_time_stamp)/mem_scale
                ...
                if it_list[0] == 'FREE:':
                    free_time_stamp = (int(it_list[2]) - start_time_stamp)/time_scale
                    if it_list[1] in malloc_dict:
                        malloc_dict[it_list[1]][3].append(free_time_stamp)
                    else:
                        print('===> !!!FREE before!!!{}:{}'.format((int(it_list[1], 16) - start_mem_addr)/mem_scale, free_time_stamp))
    ...
    for k in malloc_dict:
        ...
        v = malloc_dict[k]
        print('len v[0]: {}, v[1]: {}, v[2]: {}, v[3]: {}'.format(len(v[0]), len(v[1]), len(v[2]), len(v[3])))
